...
- **Upgrade Vercel to Pro and switch reminder cron back to hourly.** Phase 1 ships on Hobby with daily cron (`0 13 * * *` = 9am EDT). Hobby caps cron at once-per-day and function timeout at 10s; Pro ($20/mo) unlocks hourly cron + 60s timeout. Switch when reminder timing matters (clients complaining about late reminders) or when the cron loop starts hitting the 10s limit (~80+ active artists with reminders enabled). Edit `vercel.json` schedule back to `0 * * * *` after upgrading.
