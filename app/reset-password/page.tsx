...
export default async function ResetPasswordPage({
  ...
  if (!user) return redirect("/forgot-password?message=Reset+link+expired.+Please+request+a+new+one.");
    ...
    <div className="min-h-screen w-full flex flex-col md:flex-row bg-surface">
      ...
      <div className="flex-1 flex flex-col justify-center px-8 py-12 sm:px-12 md:px-16 lg:px-24 relative xl:max-w-2xl">
        ...
        <div className="w-full mt-20 md:mt-0">
          <h2 className="text-3xl font-heading tracking-tight mb-2">Set new password</h2>
          <p className="text-on-surface-variant mb-12">Choose a new password for your account.</p>

          <form className="flex flex-col w-full gap-6" action={updatePassword}>
            ...
            <div className="flex flex-col gap-2">
              <Label className="text-sm font-sans tracking-wide text-on-surface-variant" htmlFor="confirm">
                Confirm Password
              </Label>
              <Input
                id="confirm"
