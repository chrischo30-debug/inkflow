...
export async function GET() {
  ...
  const stateTemplates: EmailTemplate[] = EMAILABLE_STATES.map(state => {
    ...
    return row
      ? { id: row.id, artist_id: row.artist_id, state, name: row.name, subject: row.subject, body: row.body, auto_send: row.auto_send }
      : { state, subject: DEFAULT_EMAIL_TEMPLATES[state].subject, body: DEFAULT_EMAIL_TEMPLATES[state].body, auto_send: false };
  });

  const customTemplates: EmailTemplate[] = (rows ?? [])
    .filter(r => !r.state)
    .map(r => ({ id: r.id, artist_id: r.artist_id, state: null, name: r.name ?? 'Custom', subject: r.subject, body: r.body, auto_send: r.auto_send }));
...
export async function PUT(req: Request) {
  ...
  const { state, subject, body: templateBody, auto_send } = body as {
    state: BookingState; subject: string; body: string; auto_send: boolean;
  };
  ...
  const { error } = await supabase
    .from('email_templates')
    .upsert(
      { artist_id: user.id, state, subject, body: templateBody, auto_send },
